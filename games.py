...
class ClassicPong(Game):
    ...
    def advance(self):
        if self.is_in_end1() or self.is_in_end2():
            self.strip.set_pixel(round(self.pos), self.RETURN_WINDOW_COLOUR)
        else:
            self.strip.set_pixel(round(self.pos), Colour.clear())
        self.pos += self.speed
        if not (0 <= self.pos < len(self.strip)):
            self.game_over("Ball went out of bounds!")
            return
        self.strip.set_pixel(round(self.pos), Colour.red())
        self.strip.update()
    ...
    def send_towards_end2(self):
        return_window_size = self.RETURN_WINDOW * len(self.strip)
        multiplier = 0.5 + ((return_window_size - self.pos) / return_window_size)
        self.speed = abs(self.base_speed * multiplier)

    def send_towards_end1(self):
        return_window_size = self.RETURN_WINDOW * len(self.strip)
        multiplier = 0.5 + ((return_window_size - (len(self.strip) - self.pos)) / return_window_size)
        self.speed = -abs(self.base_speed * multiplier)
    ...
    def display_thumbnail(self):
        old_address_mode = self.strip.address_mode
        self.strip.set_addressing_mode(AddressMode.MIRRORED)
        for i in range(len(self.strip)):
            if self.is_in_end1(i) or self.is_in_end2(i):
                self.strip.set_pixel(i, self.RETURN_WINDOW_COLOUR)
        ...
        self.strip.update()
        self.strip.set_addressing_mode(old_address_mode)


class SinglePlayerPong(Game):
    TIME_UNIT = 0.025
    RETURN_WINDOW = 0.2  # What proportion of the strip can the ball be returned in?
    RETURN_WINDOW_COLOUR = Colour(0, 0, 10)
    ...
    def __init__(self, strip: Strip, input_listener: InputListener, audio=None, initial_speed=1,
                 speedup_every=5):
        super().__init__(strip, input_listener, audio=audio)
        self.strip.set_addressing_mode(AddressMode.CONTINUOUS)
        self.pos = 0
        self.initial_speed = initial_speed
        self.speed = initial_speed
        self.speedup_every = speedup_every
